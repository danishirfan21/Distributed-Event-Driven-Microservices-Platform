...
import (
	...
	"github.com/username/dist-ecommerce-go/services/inventory-service/internal/handlers"
...
func main() {
	...
	// Migrate schema
	db.AutoMigrate(&models.Product{}, &models.StockHistory{}, &models.ProcessedEvent{})
	...
	svc := service.NewInventoryService(repo, eb)
	eventHandler := handlers.NewEventHandler(svc)

	// Subscribe to events
	_, err = eb.Subscribe(events.PaymentConfirmed, "inventory-service", eventHandler.HandlePaymentConfirmed)
	if err != nil {
		log.Fatalf("failed to subscribe to PaymentConfirmed: %v", err)
	}

	log.Printf("Inventory Service started and subscribing to events")
	...
	log.Printf("Shutting down Inventory Service")
}
